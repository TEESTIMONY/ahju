    ...
    <header className="sticky top-0 z-30 w-full border-b border-black/10 bg-white/90 backdrop-blur">
      <div className="mx-auto flex w-full max-w-[86rem] items-center justify-between px-2 py-4 sm:px-3 lg:px-4">
        <Link to="/" className="flex items-center gap-2">
          ...
          <h1 className="text-base font-semibold tracking-tight text-black">AHJU</h1>
        </Link>

        <div className="flex items-center gap-3">
          {!isLoggedIn && (
            <>
              <Link to="/login" className="hidden text-sm font-medium text-black/70 transition-colors hover:text-black sm:inline">
                Sign in
              </Link>
              <Link
                to="/register"
